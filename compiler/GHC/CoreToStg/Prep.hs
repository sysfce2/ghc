...
* It is crucial that we float out String literals out of RHSs that could become
  values, e.g.,
  ...
  we don't float `FloatCase` outside of (recursive) RHSs, so we get the former
  program (this is the 'allLazyNested' test in 'wantFloatNested').
  That is what we use `FloatString` for: It is essentially a `FloatCase` which
  is always ok-to-spec/can be regarded as a non-allocating value and thus be
  floated aggressively to expose more value bindings.
-}
...
is_join_head (Lam b e) | isTyVar b = is_join_head e
...
data FloatingBind
  ...
  -- saves allocation of a thunk ('wantFloatNested'); see the Wrinkle
